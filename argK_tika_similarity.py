...
def cluster_assignment(list_of_points, centroids):
    ...
    for point in list_of_points:
        ...
        for centroid in centroids:
            ...
            #print type(point.cosTheta(centroid))
            #print point.cosTheta(centroid)
        try:
        ...
        with open(args.outJSON, "w") as jsonF:
            ...
            for key in global_minima[1]:    #clusters
                ...
                for point in global_minima[1][key]:
                    ...
                    node = {}
                    #fn1 = point.filename.split("\\")[9]
                    #FileReader1=open("E:\\Sem2\\CSCI599\\Assignment2\\Part 3\\part3\\polar8k\\test\\jsons\\"+fn1+".json",'r')
                    #file_metadata=json.loads(FileReader1.readline())
                    #node["metadata"] = json.dumps(parser.from_file(point.filename)["metadata"])
                    #node["metadata"] = json.dumps(file_metadata)
                    #node["name"] = point.filename.split('/')[-1]
                    node["name"] = point.filename
                    #node["metadata"] = json.dumps(point.features)
                    node["size"] = 3
